...
`clip` に CSS セレクタを渡すと、その要素だけを撮る。

```ts
clip: '.v-overlay__content >> nth=-1',   // いちばん手前のダイアログだけ
clip: '.v-col:has(button:has-text("新規マテリアル"))',
```

**置換表は書いてある文字列しか消せない。** 画面の一部にその章と関係の無い一覧が
写り込むと、内部のテスト名がそのまま公開画像に入る。実際にプロジェクト詳細画面の
ジョブ一覧に issue 番号を含むジョブ名が 2 件写っていた。章に必要な範囲だけを撮れば、
その一覧ごと写らない。**置換表を足して追いかけるより、写さないほうが確実。**
開発 DB のジョブは他の人の作業で増え続けるので、置換表では追い切れない。

要素が 1 つに定まらないと撮影を失敗させる（別の場所を撮るより止める）。
Playwright のセレクタ構文が使えるので、複数一致するときは `>> nth=-1` で絞る。
